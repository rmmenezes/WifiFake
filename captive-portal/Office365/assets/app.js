document.addEventListener('DOMContentLoaded', () => {
    ...
    nxt.addEventListener('click', function(event) {
        event.preventDefault(); // Impede a submissão do formulário ao clicar no 'Next'

        // Realiza a validação para a primeira seção
        validate();
        if (unameVal) {
    ...
    sig.addEventListener('click', () => {
        // Realiza a validação para a segunda seção
        validate();
        if (pwdVal) {
            // Se a validação for bem-sucedida, o formulário será submetido automaticamente
            // Não é necessário chamar event.preventDefault() ou manipular manualmente a submissão
